"""Contains a simple dataset storage and manipulation set of tools."""

import hashlib
import json


def get_ordered(dataset_id, ordered):
...
def _create_dataset_id(dataset):
    ...
    dataset_id = hashlib.sha1(json.dumps(dataset, sort_keys=True).
...
def edit_dataset(dataset_id, dataset):
    """Edit a dataset and rebuild any children."""
...
def get_dataset_by_name(name):
    """Retreive dataset by given name."""
    datasets = read_datasets()
    matching_datasets = []
    for dataset in datasets:
